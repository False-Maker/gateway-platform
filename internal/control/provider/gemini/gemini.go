...
func NewAPIKey(client *http.Client) *Provider {
	return &Provider{HTTP: providerapi.HTTPClient{Client: client}, Config: providerapi.GeminiAPIKeyProfile()}
}

func (p *Provider) Kind() string { return providerapi.KindGemini }
...
func (p *Provider) Authorize(_ context.Context, req contracts.ImportRequest) (contracts.TokenBundle, error) {
	...
	if strings.TrimSpace(req.StaticKey) == "" {
		return contracts.TokenBundle{}, fmt.Errorf("%w: Gemini API key is required", contracts.ErrInvalidContract)
	}
...
func (p *Provider) Profile(contracts.Account) contracts.UpstreamProfile {
	return contracts.UpstreamProfile{BaseURL: p.Config.APIBaseURL, Protocol: "gemini_generate", InferencePath: p.Config.InferencePath}
}

func (p *Provider) Quota(context.Context, contracts.Credential) (contracts.QuotaInfo, error) {
	// No verified account-level quota endpoint is configured for Gemini.
