...
> 注解渲染后的文本本身有意不验，理由见 A28。
>
> ~~**§A 当前没有未完成条目。**~~ —— **该判断由第八次复盘第四次推翻。**
>
> **2026-09-21 第八次复盘（对抗性代码复核 + "本轮未做"系统清扫）**：两个靶子——
> ① **对抗性复核 A17–A28 与 B5.1 的新代码**，那是全仓唯一从未被作者以外任何人读过的部分
> （第六次复盘用同一口径在 A15 里挖出过阻断级丢钱缺陷）；
> ② **系统清扫散落在「本轮未做 / 有意未做 / 未验证」段落里的欠账**——
> `EVIDENCE.md:829` 把它记为**第四类漏网机制**（写进散文段落 ⇒ 扫编号清单的人看不见），
> 项目为此吃过四次亏（B4.6 / B4.7 / E3 / B5.1），却**从未被任何一轮系统扫过**。
> 方法上沿用第四次的教训（读完整函数再下结论）与 A25 的口径（能实测的不写论证）。
>
> 结果：找出 **3 条**，编号 **A29**（单价录入把平台故障报成 400 且回泄驱动错误，P2）、
> **A30**（`moneyIntegerDigits` 误拒可存金额，P3）、**A31**（A18 的转正路径会原样恢复缺陷，P2）。
> 三条均已完成。
>
> **本次最值得记住的**：三条里有**两条是上一轮修复自己带进来的**——
> A30 是 A20 的新函数，A31 是 A18 留下的出口。而 A30 的形状尤其值得记：
> **A20 为它要防的事写了反向保护，保护存在、方向正确，只是覆盖面差一格**
> （只用了普通十进制，没测指数写法），于是它要防的事恰好发生了。
> 这是继「手写清单不是机制」（第六次 F3）之后的同一课的下一层：
> **写了反向保护也不等于测到了，保护本身也有覆盖面。**
>
> 轴②的 79 处「未做」逐条过完，**只有一条需要动，且是文档而非代码**：
> A6 与 `EVIDENCE.md:2511` 声称 per-tenant 限流与 `SessionKey` 未接线，
> 实为陈旧漂移（`run.go:122-127` 接线，`AcquireTenant` 在 `server.go:297/1256` 两路都调用），
> 与第四次复盘修的六处同类，本轮已更正。其余全部是 `[live-gate]` 或写明理由的有意取舍。
>
> **一次自我推翻**（记录下来，因为它正是第四次复盘要防的错法）：我第一次 grep
> `AcquireTenant` 只看到测试调用，差点把它记成「有表无写者」——是 `| head` 截断了输出。
> 读完整路径后推翻。
>
> **§A 当前没有未完成条目**——这句话现在第五次出现，前四次都被推翻了，
> 因此它**不应**被当作开发完成的证据。
...
证据见 `docs/EVIDENCE.md`。**未做**：~~per-tenant 限流、粘滞会话按 tenant 隔离
（`SessionKey` 仍未接线）~~ —— **该描述由第八次复盘更正（2026-09-21）**：两者均已接线，
`run.go:122-127` 从 `X-Session-Key` 派生 `AuthContext.SessionKey`，`Sticky.Lookup/Pin`
按 `(tenantID, sessionKey)` 键控，`Limiter.AcquireTenant` 在 `server.go:297`（非流式）
与 `:1256`（流式）两路都调用。属文档漂移，非缺口。
A5 `staging_only` 记录转正为 tenants 行（迁移侧后续任务）仍未做。
...
| `:57` | StarvationGateFired | warning | `SnapshotLoop.PrimeMetrics`，按 `ListSnapshotBuckets` 的 platform 列表 |
...
**`:57` StarvationGateFired —— 原以为要在「预置」与「接受漏报」之间二选一，实际不必**

它的 `platform` 取值确实来自 DB（`health.go:240`），不能静态枚举。但
`snapshot_loop.go:45` 的 `ListSnapshotBuckets` 本来就是
`SELECT DISTINCT platform,"group" FROM accounts`，**快照循环每分钟已经在读这个列表**。
于是复用它即可：`SnapshotLoop.PrimeMetrics(ctx)` 在 `run.go` 首次 `RunOnce` **之前**调用
（必须在之前——`RunOnce` 自己就会跑防饿死闸，之后预置会漏掉首轮触发的那次），
`RunOnce` 内部在取到 buckets 后每轮再预置一次，覆盖启动后才出现的新 platform。
既不新增查询，也不引入会腐烂的手写清单。

**已知残留**：某 platform 的第一个账号在启动后才出现、且闸在同一轮内即触发时，仍会漏掉那一次。
...
- ✅ `SnapshotLoop.PrimeMetrics` / `primeStarvationCounters`：按 bucket 列表去重后按 platform 预置 0；
  `run.go` 在首次 `RunOnce` 之前调用，`RunOnce` 内每轮复预置。
...
**选定的修法**：在 `Consumer.PrimeMetrics` 里把 `control_stream_reclaim_total` 预置为 0。
选它而不是给规则套 `absent()`：预置让 `== 0` 成为一次**真实比较**而不是缺失序列，
同时顺带让该指标在 A22 的形状下也成立（首次 reclaim 是 0→N 而非诞生即 N）；
`absent()` 只绕过症状，且会让规则表达式更难读。

**DoD（已满足）**

- ✅ `Consumer.PrimeMetrics` 预置 `control_stream_reclaim_total` 为 0，与 A22 的 DLQ 预置同一调用。
- ✅ 规则 `threshold_source` 写明：可触发性依赖该预置，并记录原失效机制。
- ✅ `TestPrimeMetricsCreatesZeroReclaimSeries`：首条断言是预置前该序列不存在；
  再断言真实 reclaim 能在 0 之上正常累加。
- ✅ 反向校验：删掉那行预置该测试 FAIL，还原后转绿。
...
**A26 已补完覆盖**：28 条规则全部有场景。A25 当时写的"其余 18 条"是错的，实际是 22 条
（28 减去本轮覆盖的 6 条告警）。
...
### A26 `[no-cred]` 补完全部 28 条规则的 promtool 场景，并把覆盖本身变成结构性约束

**状态：已完成（2026-09-21）。** 证据见 `docs/EVIDENCE.md` 同名小节。

A25 只覆盖了 A21–A24 动过或新增的 6 条告警，其余 22 条（A25 写成"18 条"，是错的）
共享同样的 `increase(惰性 counter)` 形状却一条都没被实测过。

**改动**

- 新增 26 个场景，覆盖剩余 22 条规则，**共 36 个场景、28 条规则全覆盖**。
- 新增 `TestEveryAlertHasAPromtoolScenario`：规则文件里每条告警都必须有场景，
  反向也查（场景指向一条已不存在的告警同样报错）。
  **这条测试不需要 promtool，因此在任何环境都跑**——新规则的门禁本身不能是可跳过的。
  这是把 A24 的教训（"覆盖范围本身会被忘记扩"）直接应用到 A25 自己身上。
- 新增 `TestEveryNegativeScenarioHasAPositiveTwin`：反向场景唯一的空跑方式是
  输入序列的指标名打错、规则因为选不中而沉默，`exp_alerts: []` 照样满足。
  要求同一条告警必须另有一个正向场景，才能证明 fixture 里的指标名真的能到达规则。

**把 A22 的"判定可容忍"变成实测**

A22 曾把 8 条规则判为"只丢首次增量、可容忍"，并在更正里写明**阈值 `> N` 不能免疫**。
本轮为其中两条补了反向场景，把那个判断从推理变成测量：

- `stream_duplicate_spike_silent_when_eleven_arrive_as_one_burst`：11 次重复挤在一个突发里，
  序列诞生即为 11、之后平稳，`increase` 恒为 0，**阈值 `> 10` 确实救不回来**。
- `console_auth_rejections_silent_when_six_arrive_as_one_burst`：同形状，阈值 `> 5`。

这两条现在是绿色断言，记录的是那 8 条规则被接受的**真实代价**，而不是对它的描述。

同理为 A22 堆 1 里已预置的两条补了反向场景（`StarvationGateFired`、
`SyntheticTerminalStateSpike`），证明预置之前它们确实沉默——即那次预置不是白做的。
...
### A29 `[no-cred]` 单价录入把平台故障报成 400，并把驱动错误原文回给调用方

**状态：已完成（2026-09-21，第八次复盘 F1）。P2。** 证据见 `docs/EVIDENCE.md` 同名小节。

**基线**：`console_billing.go:561` 的 `default:` 分支把 `InsertModelPrice` 的**任意**
错误写成 `400 + err.Error()`。实测（构造不可达 PG）：

| 端点 | 状态码 | 响应体 |
|---|---|---|
| `POST /v1/billing/prices` | **400** | `insert model price p1: failed to connect to ``user=u database=nope``: ... connection refused` |
| `POST /v1/billing/wallets/{t}/topup` | 500 | `ensure wallet failed`（细节只进日志） |

两个后果叠在同一分支：

1. **误分类**。PG 不可达、锁超时、10s context 到期，全部被告诉运营「你的输入不对」。
   4xx 不进任何 5xx 错误率口径，**平台故障在这条路径上对监控不可见**。
2. **违反本包自己写明的策略**。`console.go:344-346`：「数据库错误描述的是内部细节，
   响应没有理由携带它」——而这里把它原样回出去，含用户名、库名、地址端口。

这不是家族风格：控制台**六个写端点里只有这一个**这么做，`console_accounts.go`、
`console_held.go`、两个钱包端点都走 `c.fail` → 500。形状上它是 **A20 的镜像**
（A20 修的是「客户端错误报成 500」），且两者在同一个文件里。

**DoD 与实现**
- 新增 `ErrPriceRejected` 哨兵，`InsertModelPrice` 里**每一条**落在 PG 之前的校验都包它。
- handler 的 `default:` 改走 `c.fail` → 500，与另外五个写端点同一口径。
- 真正的 400 必须存活：`ErrPriceNotEffectiveInFuture` 保留自己的分支与自己的哨兵。
- `TestEveryPriceValidationCarriesTheRejectedSentinel` 遍历 9 种输入错误逐条断言带哨兵——
  **防的是清单腐烂**：将来新加一条校验若忘了包哨兵，会默认掉进 500，
  正是第六次复盘 F3 记的「手写清单不是机制」。

**反向校验**：把 `default:` 改回 `400 + err.Error()` → FAIL（`got 400, want 500`）；
去掉某条校验的哨兵 → FAIL 并指名该条；还原后转绿。

---

### A30 `[no-cred]` `moneyIntegerDigits` 对带指数的写法多算整数位，误拒可存金额

**状态：已完成（2026-09-21，第八次复盘 F2）。P3。** 证据见 `docs/EVIDENCE.md` 同名小节。

**基线**：A20 新增的 `moneyIntegerDigits` 用「整数部分长度 + 指数」，
**不减去被正指数吃掉的小数位**。实测：

```
0.001e28    真实整数位=26  计算=28  可存 → 被拒
0.0001e29   真实整数位=26  计算=29  可存 → 被拒
1e25        真实整数位=26  计算=26  可存 → 接受
1e30        真实整数位=31  计算=31  不可存 → 拒绝（正确）
```

**方向是安全的**：只会多算、不会少算，不存在把不可存的值漏给 PG——
**A20 要堵的 500 没有回来**。代价是 `NUMERIC(38,12)` 能存的值被误判成 400。
另有一处更小的：`+1e30` 的 `+` 没被 trim，多算一位（仍落在拒绝侧）。

**为什么没被发现**（这是本条真正值得记的）：A20 自己加了反向保护
「断言上限值（26 位整数 / 12 位小数）仍被接受，防止修复悄悄收窄可记录范围」——
**保护存在、方向正确、覆盖面差一格**，它只用了普通十进制写法，指数形式未被测过。
于是它要防的事恰好发生了。

**DoD 与实现**
- 改按**有效数字 + 小数点位置**计数：拼接整数与小数部分，指数滑动小数点，
  再减去前导零（那是小数点要走过的位置，不是值的数位）。
- `strings.TrimLeft(raw, "+-")` 一并修掉符号位。
- 全零（`0.000e30`）显式返回 0：指数怎么移都是零。
- 17 组用例覆盖普通、正负指数、符号、边界两侧与全零，
  并**按可存性**（而非数位）断言 `checkMoney` 的接受/拒绝。

**反向校验**：把算法还原成 A20 的旧式 → FAIL 并逐条列出
`0.001e28 = 29, want 26` 等五条；还原后转绿。

---

### A31 `[no-cred]` A18 停放了账号，但它指的那条转正路径会原样恢复缺陷

**状态：已完成（2026-09-21，第八次复盘 F3）。P2。** 证据见 `docs/EVIDENCE.md` 同名小节。

**基线**：A18 的判断正确——能力集读不出来的账号必须停下，因为 `chooser.go:301` 的
`supports()` 把**空能力集读作「匹配任意模型」**，谁都描述不了的账号会成为池子里最宽松的。
停放实现也对。问题在转正侧，逐段验证：

1. A18 把区分信息写进 `migration_records.conversion`（`disabled_because` / `source_status`）。
2. **控制台完全不暴露 `conversion`**，而账号看板 `accountView` 有 `excluded_models`
   却**没有 `capabilities`**。运营看到的只是 `status: disabled`——与「源渠道本来就禁用」
   一模一样。A18 说这个歧义「靠 `conversion.source_status` 区分」，
   但**产品里没有任何地方能读到它**。
3. A18 明写转正走 `POST /v1/accounts/{id}/status`。该端点只校验
   `status ∈ {active, disabled}` 与 `reason` 非空，**对能力集零前置条件**。
4. 全 control 包 `capabilities` **只被读、从无写入点**（仅 `snapshot_loop.go:73/139`），
   控制台也没有对应端点。**能力集根本修不了。**

于是运营能执行的唯一动作，就是把那个账号原样放回池子，并且它依然匹配所有模型。
**A18 修的洞，从 A18 自己指的那扇门走回来了**，而且无声——账号在看板上完全健康。

**DoD 与实现**
- **看板暴露 `capabilities`**（查询、扫描、`accountView` 三处）。
  **刻意不加 `omitempty`**：缺键与空列表会读成一样，而空列表正是最该被看见的状态。
- **转正需显式确认**：`status=active` 且能力集为空且无 `allow_unrestricted` → **409**，
  文案说明「空集在 gateway 眼里是"服务所有模型"而不是"不服务"」并给出该字段。
- **判定收在事务内**（`applyAccountAction` 新增 `guard` 形参），
  消除「读行」与「改行」之间的窗口，与 `handleTopup` 依赖外键而非前置存在性检查同一理由。
- **只拦转正，不拦停用**：停用正是账号被停放的方式，事故中要求填标志位是错的。
- 判定抽成 `needsUnrestrictedAcknowledgement`，使它在无 PG 环境也可被测。

**有意不做（本条的主要判断）**
- **不新造状态词**（沿用 A18 的理由：`console_accounts.go` 警告第三种状态会让账号
  「既不可调度也不显式可见地坏掉」）。
- **不加修能力集的端点**。那要决定「运营手填模型列表」还是「重跑迁移转换」，
  是设计决定不是补洞；本条只保证**恢复缺陷这件事必须是显式的**。
  空能力集仍然合法——它是 apikey 渠道表达「无限制」的方式，所以这里是确认而非禁止，
  与 E2 对 fixture executor 的「两个独立确认」同形。

**反向校验（三条）**：去掉 guard → 集成测试 `got 200, want 409` 且单元断言
`= false, want true`；让 guard 连 `disabled` 也拦 → 反向保护 FAIL（`got 409, want 200`）；
