...
- [x] **平台级速率告警**:`PlatformSignals` 按平台 60s 窗口聚合 `forbidden_transport` / `blocked` 的不同账号数,≥2 即置 `control_platform_alert{provider}` 网关指标——这是 R4(TLS 指纹时效)与出口 IP 被标记的**唯一探测机制**(总览 §6.3.1 规则 2)。Prometheus 告警规则**已由 E1 交付**;通知通道(谁收、怎么收)仍属部署侧。§6.3.1 要求的**处置**(出口路径 5–10s 短路)**已由 A14 交付**(2026-09-14,gateway 侧 `internal/gateway/chooser.go` 的出口短路表,与本条同一口径:60s 窗口内 ≥2 个不同账号);短路是**进程内**的,跨实例的聚合仍由本条的 `PlatformSignals` 承担。
